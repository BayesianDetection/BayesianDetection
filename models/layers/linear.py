...
class RandLinear(nn.Module):
    ...
    def reset_parameters(self):
        ...
        #init.uniform_(self.mu_weight, -stdv, stdv)
        self.mu_weight.data.uniform_(-stdv, stdv)
        #init.constant_(self.sigma_weight, self.init_s)
        self.sigma_weight.data.fill_(self.init_s)
        #init.zeros_(self.eps_weight)
        self.eps_weight.data.zero_()
        if self.mu_bias is not None:
            #init.uniform_(self.mu_bias, -stdv, stdv)
            #init.constant_(self.sigma_bias, self.init_s)
            #init.zeros_(self.eps_bias)
